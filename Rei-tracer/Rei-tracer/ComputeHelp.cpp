...
bool ComputeShader::Init(LPCWSTR shaderFile, LPCWSTR blobFileAppendix, char* pFunctionName, D3D10_SHADER_MACRO* pDefines,
	ID3D11Device* d3dDevice, ID3D11DeviceContext*d3dContext)
{
	...
	wchar_t blobFilename[300];
	if(blobFileAppendix != nullptr)
	{
		size_t l1 = wcslen(shaderFile);
		size_t l2 = wcslen(wcsrchr(shaderFile, _T('.')));
		wcsncpy_s(blobFilename, shaderFile, l1 - l2);
		
		wcscat_s(blobFilename, L"_");
		wcscat_s(blobFilename, blobFileAppendix);
		wcscat_s(blobFilename, L".blob");

		//MessageBox(0, blobFilename, L"", 0);

		char narrow[300];
		wcstombs(narrow, blobFilename, 300);
		_tfopen_s(&fShaderBlob, narrow, _T("rb"));
	}
	...
	if(fShaderBlob == nullptr)
	{
		...
		if(hr == S_OK)
		{
			if(blobFileAppendix != nullptr)
			{
				char narrow[300];
				wcstombs(narrow, blobFilename, 300);
				_tfopen_s(&fShaderBlob, narrow, _T("wb"));
...
ComputeShader* ComputeWrap::CreateComputeShader(LPCWSTR shaderFile, LPCWSTR blobFileAppendix, char* pFunctionName, D3D10_SHADER_MACRO* pDefines)
{
